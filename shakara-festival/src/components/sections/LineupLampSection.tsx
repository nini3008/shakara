...
export function LineupLampSection({ children }: { children?: React.ReactNode }) {
  return (
    <OrangeLampContainer>
      {/* Feature section nested under the lamp */}
      <motion.div
        ...
        {children || <div className="text-center text-white/50">Loading...</div>}
      </motion.div>
    </OrangeLampContainer>
  );
}
...
const OrangeLampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[100vh] flex-col items-center justify-start overflow-hidden bg-slate-950 w-full z-0",
        className
      )}
    >
      <div className="relative flex w-full h-56 scale-y-125 items-center justify-center isolate z-0 ">
        <motion.div
      ...
      <div className="relative z-50 flex -translate-y-20 flex-col items-center w-full">
        {children}
      </div>
    </div>
  );
